...
[**`ENUMFILES`**](modify_com#enumfiles) 指令的第4参数可传入字符串型可引用数组、列表、哈希列表。
...
新增 [**`ARRAYREVERSE`**](new_com#arrayreverse) 指令，用于将数组或列表的指定范围内的元素进行反序排列。
...
- 请参阅 [**`语法、指令与程序的兼容性变动`**](/#CompatibilityChanges) 中的 `ERD键词功能示例` 以了解这些改进功能的用法。

[**`TRYTOINT`**](new_com#trytoint) 指令新增第2参数 `outValue`，用于指定接收转换结果的整数型变量，省略时将使用 `RESULT:1` 来接收转换结果。
