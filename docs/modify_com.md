...
**`void SETANIMETIMER int interval, int duration(, int durationOverlap = 0)`**
...
- **int interval**
  - 指定动画计时器的刷新间隔，单位为毫秒。
- **int duration**
  - 指定动画计时器的持续时间，单位为毫秒。持续时间过后会自动停止刷新动画。
- **int durationOverlap = 0**
  - 指定是否将当前输入的持续时间与剩余的持续时间重叠，即用于确保动画不会被过短的持续时间所中断，可省略。
    - `0` = 重设持续时间
    - `非0` = 重叠持续时间
:::
...
新增第3参数 `delay`，用于指定延时播放的持续时间。  
新增第4参数 `fadeIn`，用于指定淡入效果的持续时间。  
新增第5参数 `groupID`，用于指定本次的播放组号，可配合 [**`STOPBGM`**](modify_com#stopbgm) 指令来停止相同组号的所有背景音乐。
...
新增一种专用于控制背景音乐的播放状态的参数格式，可配合 [**`PAUSEBGM`**](new_com#pausebgm) 指令实现背景音乐的暂停与恢复播放。
...
- **int fadeIn = 0**
  - 指定淡入效果的持续时间，单位为 `ms(毫秒)`，输入值 `省略` 或 `小于等于0` 时无效果，最大值为 `10000`。  
    该参数将会影响所有指定组号的背景音乐。
- **int groupID**
  - 指定想要恢复播放的组号，省略该参数时即为恢复播放所有背景音乐。
:::
...
新增第2参数 `volume`，用于指定本次的播放音量。  
新增第3参数 `delay`，用于指定本次的播放延时。  
新增第4参数 `groupID`，用于指定本次的播放组号，可配合 [**`STOPSOUND`**](modify_com#stopsound) 指令来停止相同组号的所有音效。
...
新增 `fadeOut` 参数，输入值 `大于0` 时可让背景音乐停止时带有淡出效果。  
新增 `groupID` 参数，用于指定想要停止播放的组号，省略该参数时即为停止播放所有背景音乐。
...
- **int fadeOut = 0**
  - 指定淡出效果的持续时间，单位为 `ms(毫秒)`，输入值 `省略` 或 `小于等于0` 时无效果，最大值为 `10000`。  
    该参数将会影响所有指定组号的背景音乐。
- **int groupID**
  - 指定想要停止播放的组号，省略该参数时即为停止播放所有背景音乐。
:::
...
新增 `groupID` 参数，用于指定想要停止播放的组号，省略该参数时即为停止播放所有音效。

:::tip[参数]
- **int groupID**
  - 指定想要停止播放的组号，省略该参数时即为停止播放所有音效。
:::

:::note[使用例]
```
PLAYSOUND "MySOUND1", , 1	; 播放音效“MySOUND1”，组号设为1
PLAYSOUND "MySOUND2", , 2	; 播放音效“MySOUND2”，组号设为2
STOPSOUND 2			; 停止播放所有组号为2的音效
```
:::
