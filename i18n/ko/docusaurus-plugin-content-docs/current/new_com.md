...
이 명령은 배열 또는 리스트의 지정된 범위 내 요소를 역순으로 정렬하는 데 사용됩니다.

:::tip[매개변수]
- **any Array_List**
  - 처리할 모든 참조 가능 배열 또는 리스트를 지정합니다.
    - 다차원 배열의 경우: 마지막 차원의 요소만 처리하며, 이전 차원 인덱스 값을 수동으로 지정해야 합니다.
- **int start = 0**
  - 시작 인덱스를 지정합니다.
- **int end = lastDimLength**
  - 종료 인덱스+1을 지정합니다. 생략 시 배열의 마지막 차원 길이 또는 리스트의 총 길이가 사용됩니다.
:::
...
PRINTSL STRJOIN(LOCAL)		"0,1,5,4,3,2" 출력
```
:::
...
이 명령은 배열 또는 리스트 내 요소 간의 빈 값을 정리하여 간격 없이 요소가 연속된 컬렉션을 얻을 수 있습니다.

:::tip[매개변수]
- **any Array_List**
  - 정리할 모든 참조 가능 배열 또는 리스트를 지정합니다.
    - 다차원 배열의 경우: 마지막 차원의 요소만 처리하며, 이전 차원 인덱스 값을 수동으로 지정해야 합니다.
    - 리스트의 경우: 정리가 완료된 후 빈 요소는 제거됩니다.
- **int start = 0**
  - 정리를 시작할 인덱스를 지정합니다.
- **int end = lastDimLength**
  - 정리를 종료할 인덱스+1을 지정합니다. 생략 시 배열의 마지막 차원 길이 또는 리스트의 총 길이가 사용됩니다.
- **same emptyVal**
  - 처리 시 빈 값으로 간주될 숫자 값 또는 문자열을 지정합니다. 값 유형은 첫 번째 매개변수의 값 유형과 일치해야 합니다. 생략 가능 (`0` 또는 `빈 문자열`).
:::

:::tip[반환 값]
- **RESULT:0**
  - 지정된 범위 내에서 정리가 완료된 후의 요소 개수를 반환합니다.
:::
...
NinePatch 규칙에 따라 지정된 점 아홉 그림(NinePatch)을 늘리고 그리는 데 사용됩니다. 이 명령으로 지정하는 점 아홉 그림에는 가장자리 검은 막대가 포함될 필요가 없습니다. 점 아홉 그림의 중심 사각형 영역을 결정하는 매개변수를 전달하기만 하면 점 아늉 그림 주변의 늘림 영역을 자동으로 구분할 수 있습니다.

:::tip[매개변수]
- **int GID**
  - 이미지 ID를 지정합니다.
- **any ninePatchImage**
  - 점 아홉 그림의 이미지 ID 또는 스프라이트 이름을 지정합니다. 지정된 점 아홉 그림에는 가장자리 검은 막대가 포함될 필요가 없습니다.
- **int centerX**
  - 점 아홉 그림의 중심 사각형 X 위치를 지정합니다.
- **int centerY**
  - 점 아홉 그림의 중심 사각형 Y 위치를 지정합니다.
- **int centerWidth**
  - 점 아홉 그림의 중심 사각형 너비를 지정합니다.
- **int centerHeight**
  - 점 아홉 그림의 중심 사각형 높이를 지정합니다.
- **int destX**
  - 대상 X 위치를 지정합니다. 생략 시 점 아홉 그림은 전체 대상 이미지로 늘어납니다.
- **int destY**
  - 대상 Y 위치를 지정합니다. 생략 시 점 아홉 그림은 전체 대상 이미지로 늘어납니다.
- **int destWidth**
  - 대상 너비를 지정합니다. 생략 시 점 아홉 그림은 전체 대상 이미지로 늘어납니다. `음수`를 입력하여 뒤집힌 이미지를 그릴 수 있습니다.
- **int destHeight**
  - 대상 높이를 지정합니다. 생략 시 점 아홉 그림은 전체 대상 이미지로 늘어납니다. `음수`를 입력하여 뒤집힌 이미지를 그릴 수 있습니다.
:::

:::tip[반환 값]
- **RESULT:0**
  - 그래픽이 성공적으로 그려졌는지 나타냅니다. 성공 시 `0이 아닌 값`을 반환합니다. 지정된 이미지 또는 점 아홉 그림이 생성되지 않은 경우 `0`을 반환합니다.
:::
...
현재 재생 중인 배경 음악 이름 목록을 가져옵니다.

:::tip[매개변수]
- **str Array_List_HashList**
  - 현재 재생 중인 배경 음악 이름을 받을 문자열형 참조 가능 배열, 리스트, 해시 리스트를 지정합니다.
    - 리스트 및 해시 리스트의 경우: 변수의 기존 내용은 지워지고 새 내용으로 채워집니다.
- **int groupID**
  - 동일한 그룹 번호의 배경 음악 이름을 가져오려는 그룹 번호를 지정합니다. 이 매개변수를 생략하면 모든 배경 음악 이름을 가져옵니다.
:::

:::tip[반환 값]
- **RESULT:0**
  - 가져온 현재 재생 중인 배경 음악 이름 수를 반환합니다.  
    가져온 수는 배열 길이와 해시 리스트 특성에 따라 영향을 받을 수 있습니다.
:::
...
현재 재생 중인 배경 음악을 일시 정지합니다. [**`PLAYBGM`**](modify_com#playbgm) 명령과 함께 사용하여 일시 정지된 배경 음악의 재생을 재개할 수 있습니다.
...
- **int groupID**
  - 일시 정지하려는 그룹 번호를 지정합니다. 이 매개변수를 생략하면 모든 배경 음악이 일시 정지됩니다.
:::
