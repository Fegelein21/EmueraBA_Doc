...
이 매개변수 형식의 네 번째 매개변수는 문자열형 참조 가능 배열, 리스트, 해시 리스트를 전달할 수 있습니다.
...
- **str pattern**
  - 정규 표현식 규칙을 지정합니다.
- **int groupCount**
  - 일치하는 부분 표현식 그룹 수를 받을 숫자형 변수를 지정합니다.
- **str Array_List_HashList**
  - 모든 일치 결과를 받을 문자열형 참조 가능 배열, 리스트, 해시 리스트를 지정합니다.
    - 다차원 배열의 경우: 마지막 차원의 요소만 처리하며, 이전 차원 인덱스 값을 수동으로 지정해야 합니다.
    - 리스트 및 해시 리스트의 경우: 변수의 기존 내용은 지워지고 새 내용으로 채워집니다.
:::
...
이 명령의 첫 번째 매개변수 `Array_List_HashList`는 모든 유형의 참조 가능 배열, 리스트, 해시 리스트를 전달할 수 있습니다.

:::tip[매개변수]
- **any Array_List_HashList**
  - 문자열을 병합할 모든 유형의 참조 가능 배열, 리스트, 해시 리스트를 지정합니다.
    - 다차원 배열의 경우: 마지막 차원의 요소만 처리하며, 이전 차원 인덱스 값을 수동으로 지정해야 합니다.
- **str delimiter = ","**
  - 문자열 병합 시 사용할 구분자를 지정합니다.
- **int start = 0**
  - 병합을 시작할 인덱스를 지정합니다.
- **int count = lastDimLength**
  - 병합할 요소 수를 지정합니다. 생략 시 배열의 마지막 차원 길이 또는 리스트의 총 길이가 사용됩니다.
:::
...
첫 번째 매개변수 `name`은 오디오 이름 입력만 지원합니다. 오디오 파일 경로로 재생하려면 먼저 [**`AUDIOCREATEFROMFILE`**](new_com#audiocreatefromfile) 명령을 사용하여 오디오를 생성하세요.  
내장 오디오 리소스 추가 방법은 [**`오디오 기능`**](/#AudioFunc) 섹션을 참조하세요.

두 번째 매개변수 `volume`을 추가하여 이번 재생 볼륨을 지정할 수 있습니다.  
세 번째 매개변수 `delay`를 추가하여 지연 재생 지속 시간을 지정할 수 있습니다.  
네 번째 매개변수 `fadeIn`을 추가하여 페이드 인 효과 지속 시간을 지정할 수 있습니다.  
다섯 번째 매개변수 `groupID`를 추가하여 이번 재생 그룹 번호를 지정할 수 있으며, [**`STOPBGM`**](modify_com#stopbgm) 명령과 함께 사용하여 동일한 그룹 번호의 모든 배경 음악을 중지할 수 있습니다.
...
- **int delay = 0**
  - 지연 재생 지속 시간을 지정합니다. 단위는 `ms(밀리초)`입니다. 입력 값이 `생략` 또는 `0 이하`일 경우 효과 없음, 최대값 `10000`.
- **int fadeIn = 0**
  - 페이드 인 효과 지속 시간을 지정합니다. 단위는 `ms(밀리초)`입니다. 입력 값이 `생략` 또는 `0 이하`일 경우 효과 없음, 최대값 `10000`.
- **int groupID = 0**
  - 이번 재생 그룹 번호를 지정합니다. 생략 가능 `(0)`.
:::

**`int PLAYBGM (int delay = 0, int fadeIn = 0, int groupID)`**

배경 음악 재생 상태를 제어하기 위한 새로운 매개변수 형식을 추가했습니다. 이는 [**`PAUSEBGM`**](new_com#pausebgm) 명령과 함께 사용하여 배경 음악 일시 정지 및 재개 재생을 구현할 수 있습니다.

:::tip[매개변수]
- **int delay = 0**
  - 지연 재생 지속 시간을 지정합니다. 단위는 `ms(밀리초)`입니다. 입력 값이 `생략` 또는 `0 이하`일 경우 효과 없음, 최대값 `10000`.  
    이 매개변수는 지정된 그룹 번호의 모든 배경 음악에 영향을 미칩니다.
- **int fadeIn = 0**
  - 페이드 인 효과 지속 시간을 지정합니다. 단위는 `ms(밀리초)`입니다. 입력 값이 `생략` 또는 `0 이하`일 경우 효과 없음, 최대값 `10000`.  
    이 매개변수는 지정된 그룹 번호의 모든 배경 음악에 영향을 미칩니다.
- **int groupID**
  - 재생을 재개하려는 그룹 번호를 지정합니다. 이 매개변수를 생략하면 모든 배경 음악이 재생을 재개합니다.
:::

:::tip[반환 값]
- **RESULT:0**
  - 재생 성공 여부를 나타냅니다. 성공 시 `0이 아닌 값`을 반환합니다. 오디오가 존재하지 않거나 오디오 로딩 오류 시 `0`을 반환합니다.
:::
...
두 번째 매개변수 `volume`을 추가하여 이번 재생 볼륨을 지정할 수 있습니다.  
세 번째 매개변수 `delay`를 추가하여 이번 재생 지연을 지정할 수 있습니다.  
네 번째 매개변수 `groupID`를 추가하여 이번 재생 그룹 번호를 지정할 수 있으며, [**`STOPSOUND`**](modify_com#stopsound) 명령과 함께 사용하여 동일한 그룹 번호의 모든 사운드 효과를 중지할 수 있습니다.
...
- **int volume**
  - 이번 재생 볼륨을 지정합니다. 생략 가능 `(오디오의 사전 설정 볼륨 사용)`.
- **int delay = 0**
  - 이번 재생 지연을 지정합니다. 단위는 밀리초입니다. 생략 가능 `(0)`.
- **int groupID = 0**
  - 이번 재생 그룹 번호를 지정합니다. 생략 가능 `(0)`.
:::
...
`fadeOut` 매개변수를 추가했습니다. 입력 값이 `0보다 크면` 배경 음악이 중지될 때 페이드 아웃 효과를 가질 수 있습니다.  
`groupID` 매개변수를 추가하여 중지하려는 그룹 번호를 지정할 수 있습니다. 이 매개변수를 생략하면 모든 배경 음악이 중지됩니다.

:::tip[매개변수]
- **int fadeOut = 0**
  - 페이드 아웃 효과 지속 시간을 지정합니다. 단위는 `ms(밀리초)`입니다. 입력 값이 `생략` 또는 `0 이하`일 경우 효과 없음, 최대값 `10000`.  
    이 매개변수는 지정된 그룹 번호의 모든 배경 음악에 영향을 미칩니다.
- **int groupID**
  - 중지하려는 그룹 번호를 지정합니다. 이 매개변수를 생략하면 모든 배경 음악이 중지됩니다.
:::

:::note[사용 예시]
```
STOPBGM	1500			; 배경 음악을 1500ms 페이드 아웃 효과와 함께 중지
```
:::
...
`groupID` 매개변수를 추가하여 중지하려는 그룹 번호를 지정할 수 있습니다. 이 매개변수를 생략하면 모든 사운드 효과가 중지됩니다.

:::tip[매개변수]
- **int groupID**
  - 중지하려는 그룹 번호를 지정합니다. 이 매개변수를 생략하면 모든 사운드 효과가 중지됩니다.
:::

:::note[사용 예시]
```
PLAYSOUND "MySOUND1", , 1	; 사운드 효과 "MySOUND1" 재생, 그룹 번호를 1로 설정
PLAYSOUND "MySOUND2", , 2	; 사운드 효과 "MySOUND2" 재생, 그룹 번호를 2로 설정
STOPSOUND 2			; 그룹 번호 2의 모든 사운드 효과 중지
```
:::
...
이 명령의 네 번째 매개변수는 문자열형 참조 가능 배열, 리스트, 해시 리스트를 전달할 수 있습니다.

이 명령으로 얻은 파일 경로의 백슬래시 `\\`는 슬래시 `/`로 대체되었습니다.

:::tip[매개변수]
- **str dir**
  - 파일을 검색할 디렉터리를 지정합니다.
- **str pattern = "*"**
  - 파일명 일치 규칙을 지정합니다. 생략 가능 `("*"，임의 파일명 일치)`.
- **int option = 0**
  - 하위 디렉터리의 파일을 검색할지 여부를 지정합니다. 입력 값이 `0`일 경우 현재 디렉터리의 파일만 검색합니다. 생략 가능 `(0)`.
- **str Array_List_HashList = RESULTS**
  - 찾은 파일 경로를 받을 문자열형 참조 가능 배열, 리스트, 해시 리스트를 지정합니다. 생략 가능 `(RESULTS 배열 사용)`.  
    주의: 찾은 파일 경로에는 원본 디렉터리 경로가 포함됩니다.
    - 다차원 배열의 경우: 마지막 차원의 요소만 처리하며, 이전 차원 인덱스 값을 수동으로 지정해야 합니다.
    - 리스트 및 해시 리스트의 경우: 변수의 기존 내용은 지워지고 새 내용으로 채워집니다.
:::
