...
- **same emptyVal**
  - 処理中に空の値として扱われる数値または文字列を指定します。値のタイプは最初のパラメータの値のタイプと一致している必要があります。省略可能です (`0` または `空文字列`)。
:::
...
NinePatchルールに従って指定された点九図を伸縮して描画するために使用されます。このコマンドで指定する点九図には、端の黒いバーを含める必要はありません。点九図の中央矩形領域を決定するためのパラメータを渡すだけで、点九図の周囲の伸縮領域を自動的に区別できます。

:::tip[パラメータ]
- **int GID**
  - 画像IDを指定します。
- **any ninePatchImage**
  - 点九図の画像IDまたはスプライト名を指定します。指定された点九図には、端の黒いバーを含める必要はありません。
- **int centerX**
  - 点九図の中央矩形のX位置を指定します。
- **int centerY**
  - 点九図の中央矩形のY位置を指定します。
...
- **int centerHeight**
  - 点九図の中央矩形の高さを指定します。
- **int destX**
  - ターゲットX位置を指定します。省略された場合、点九図はターゲット画像全体に伸縮されます。
- **int destY**
  - ターゲットY位置を指定します。省略された場合、点九図はターゲット画像全体に伸縮されます。
- **int destWidth**
  - ターゲット幅を指定します。省略された場合、点九図はターゲット画像全体に伸縮されます。`負数` を入力して反転画像を描画できます。
- **int destHeight**
  - ターゲット高さを指定します。省略された場合、点九図はターゲット画像全体に伸縮されます。`負数` を入力して反転画像を描画できます。
:::

:::tip[戻り値]
- **RESULT:0**
  - グラフィックが正常に描画されたかどうかを示します。成功した場合は `非ゼロ` を返します。指定された画像または点九図が作成されていない場合は `0` を返します。
:::
...
現在再生中のバックグラウンドミュージック名のリストを取得します。

:::tip[パラメータ]
- **str Array_List_HashList**
  - 現在再生中のバックグラウンドミュージック名を受信する文字列型参照可能配列、リスト、ハッシュリストを指定します。
    - リストおよびハッシュリストの場合：変数内の元のコンテンツはクリアされ、新しいコンテンツで埋められます。
- **int groupID**
  - 同じグループ番号のバックグラウンドミュージック名を取得したいグループ番号を指定します。このパラメータを省略すると、すべてのバックグラウンドミュージック名が取得されます。
:::

:::tip[戻り値]
- **RESULT:0**
  - 取得された現在再生中のバックグラウンドミュージック名の数を返します。  
    取得される数は、配列の長さとハッシュリストの特性によって影響を受ける可能性があります。
:::
...
現在再生中のバックグラウンドミュージックを一時停止します。 [**`PLAYBGM`**](modify_com#playbgm) コマンドと組み合わせて、一時停止したバックグラウンドミュージックの再生を再開できます。
...
- **int groupID**
  - 一時停止したいグループ番号を指定します。このパラメータを省略すると、すべてのバックグラウンドミュージックが一時停止します。
:::
